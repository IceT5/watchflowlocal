...
async def fetch_pr_signals(state: AnalysisState) -> AnalysisState:
    ...
    try:
        ...
        pr_limit = min(state.pr_limit, 200)
        
        if pr_limit > 20:
            pr_nodes, pr_warning = await github_client.fetch_pr_hygiene_stats_paginated(
                owner=owner, repo=repo_name, user_token=state.user_token, installation_id=None, limit=pr_limit
            )
        else:
        ...
        logger.info("debug_pr_nodes_fetched", count=len(pr_nodes), requested_limit=pr_limit)
...
- AI Generated Rate: {ai_rate_text} (low-signal PRs)"""
    ...
    max_retries = 3
    retry_delay = 2.0
    fallback_reason = "Unknown error occurred"
    
    for attempt in range(max_retries):
        try:
            llm = get_chat_model(agent="repository_analysis", temperature=0.4)
            
            logger.info("llm_instance_created", repo=repo_name, model=llm.model_name if hasattr(llm, 'model_name') else 'unknown', attempt=attempt + 1)

            class RecommendationsList(pydantic.BaseModel):
                recommendations: list[RuleRecommendation]

            # Try structured output first, fall back to JSON mode if not supported
            try:
                structured_llm = llm.with_structured_output(RecommendationsList, method="function_calling")
            except Exception as struct_error:
                logger.warning("structured_output_not_supported", error=str(struct_error), falling_back="json_mode")
                structured_llm = llm.with_structured_output(RecommendationsList, method="json_mode")
            ...
            logger.error("llm_auth_error", repo=repo_name, error=str(e), error_type="authentication")
            fallback_reason = f"AI provider authentication failed. Check your API key: {e.__class__.__name__}"
            break
        except openai.RateLimitError as e:
            logger.warning("llm_rate_limit", repo=repo_name, error=str(e), attempt=attempt + 1, max_retries=max_retries)
            fallback_reason = f"AI provider rate limit exceeded"
            if attempt < max_retries - 1:
                await asyncio.sleep(retry_delay * (attempt + 1))
                continue
        except openai.APITimeoutError as e:
            logger.error("llm_timeout", repo=repo_name, error=str(e), attempt=attempt + 1)
            fallback_reason = f"AI provider request timed out"
            if attempt < max_retries - 1:
                await asyncio.sleep(retry_delay * (attempt + 1))
                continue
        except openai.APIConnectionError as e:
            logger.error("llm_connection_error", repo=repo_name, error=str(e), attempt=attempt + 1)
            fallback_reason = f"AI provider connection failed: {str(e)}"
            if attempt < max_retries - 1:
                await asyncio.sleep(retry_delay * (attempt + 1))
                continue
        except openai.BadRequestError as e:
            logger.error("llm_bad_request", repo=repo_name, error=str(e), error_type="bad_request")
            fallback_reason = f"AI provider rejected request (check model compatibility): {str(e)}"
            break
        except openai.OpenAIError as e:
            logger.error("rule_generation_failed", repo=repo_name, error=str(e), error_type="llm_provider_error", attempt=attempt + 1)
            fallback_reason = f"AI provider error: {e.__class__.__name__} - {str(e)}"
            if attempt < max_retries - 1:
                await asyncio.sleep(retry_delay * (attempt + 1))
                continue
            ...
            logger.error("llm_value_error", repo=repo_name, error=str(e), attempt=attempt + 1)
            fallback_reason = f"LLM response validation error: {str(e)}"
            if attempt < max_retries - 1:
                await asyncio.sleep(retry_delay * (attempt + 1))
                continue
        except Exception as e:
            logger.error("rule_generation_failed", repo=repo_name, error=str(e), error_type="unknown_error", exc_info=True, attempt=attempt + 1)
    ...
    logger.info("using_fallback_rules", repo=repo_name, reason=fallback_reason)
