...
class GitHubClient:
    ...
    def _decode_private_key() -> str:
        try:
            if not config.github.private_key:
                logger.warning("No GitHub App private key configured. Running in public-only mode.")
                return ""
            decoded_key = base64.b64decode(config.github.private_key).decode("utf-8")
            return decoded_key
        except Exception as e:
            logger.warning(f"Failed to decode private key: {e}. Running in public-only mode.")
            return ""
    ...
    async def fetch_pr_hygiene_stats_paginated(
        self,
        owner: str,
        repo: str,
        user_token: str | None = None,
        ...
        """
        Fetches PR statistics for hygiene analysis using GraphQL with pagination.
        ...
        Args:
            ...
            user_token: Optional GitHub Personal Access Token for authenticated requests.
            installation_id: Optional GitHub App installation ID for authenticated requests.
            limit: Maximum number of PRs to fetch (default 200).

        Returns:
            Tuple of (pr_nodes, warning_message). warning_message is None if successful, or a string describing the issue.
        """
        _PR_HYGIENE_QUERY_PAGINATED = """
        query PRHygiene($owner: String!, $repo: String!, $first: Int, $after: String) {
          repository(owner: $owner, name: $repo) {
            pullRequests(first: $first, after: $after, states: [MERGED, CLOSED], orderBy: {field: UPDATED_AT, direction: DESC}) {
              pageInfo {
                hasNextPage
                endCursor
              }
        ...
        try:
            while has_next_page and len(all_nodes) < limit:
                variables: dict[str, Any] = {"owner": owner, "repo": repo, "first": page_size}
                if cursor:
                    variables["after"] = cursor

                data = await self.execute_graphql(
                    _PR_HYGIENE_QUERY_PAGINATED, variables, user_token=user_token, installation_id=installation_id
                )
            ...
            logger.info("pr_hygiene_fetch_completed", owner=owner, repo=repo, total_prs=len(all_nodes))
            return all_nodes[:limit], warning_msg
            ...
            if is_rate_limit:
                if all_nodes:
                    return all_nodes, f"GitHub API rate limit reached. Fetched {len(all_nodes)} PRs before limit. Add a GitHub token for higher rate limits."
                return [], f"GitHub API rate limit exceeded. Unable to fetch PR data. Add a GitHub Personal Access Token for higher rate limits."
            
            logger.error("Failed to fetch PR hygiene stats with pagination", error=str(e))
            return [], f"Failed to fetch PR data: {str(e)}"
    ...
    async def fetch_all_pull_requests(
        ...
        """
        Fetches all pull requests using REST API with pagination.
        Used for batch PR analysis.
        ...
        try:
            ...
            logger.info("fetch_all_prs_completed", repo=repo_full_name, total_prs=len(all_prs[:limit]))
            return all_prs[:limit]

        except Exception as e:
            logger.error("fetch_all_prs_error", repo=repo_full_name, error=str(e))
            return all_prs
