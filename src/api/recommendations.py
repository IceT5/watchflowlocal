...
# --- Mock Endpoints for Testing ---  # Work without AI providers


@router.post("/recommend-test", response_model=AnalysisResponse)
async def recommend_rules_test(request: Request, payload: AnalyzeRepoRequest, user: User | None = Depends(get_current_user_optional)) -> AnalysisResponse:
    """
    Mock endpoint for testing rule generation without requiring AI providers.
    Returns sample rules based on common repository patterns.
    """
    # Generate mock rules based on the repository URL
    repo_name = str(payload.repo_url).replace("https://github.com/", "").replace("http://github.com/", "")
    
    # Sample rules that would be generated based on different patterns
    sample_rules = {
        "default": [
            {
                "description": "Pull requests must reference an issue number",
                "enabled": True,
                "severity": "medium",
            ...
            {
                "description": "PR titles should follow conventional commits",
                "enabled": True,
                "severity": "medium", 
                "event_types": ["pull_request"],
            ...
            {
                "description": "Maximum 500 lines changed per PR",
                "enabled": True,
                "severity": "medium",
                "event_types": ["pull_request"],
        ...
        "frontend": [
            {
                "description": "Component changes require review",
                "enabled": True,
                "severity": "high",
                "event_types": ["pull_request"],
                "parameters": {
                    "require_code_owner_reviewers": True,
                    "title_pattern": "^feat\\(.*\\)|^fix\\(.*\\)"
                }
            }
        ],
        "backend": [
    ...
    # Determine which rule set to use based on repo name
    rules_to_use = sample_rules["default"]
    if "frontend" in repo_name.lower():
        rules_to_use = sample_rules["frontend"]
    elif "backend" in repo_name.lower() or "api" in repo_name.lower():
        rules_to_use = sample_rules["backend"]
    
    # Generate rules YAML
    import yaml
    rules_output = {"rules": rules_to_use}
    ...
    return AnalysisResponse(
        ...
        pr_plan={
            "title": f"Add Watchflow governance rules ({len(rules_to_use)} rules)",
            "body": f"This PR adds automated governance rules for {repo_name} based on repository analysis.",
            "branch_name": "watchflow/rules",
            "base_branch": "main",
            "file_path": ".watchflow/rules.yaml",
            "commit_message": f"chore: add Watchflow governance rules ({len(rules_to_use)} rules)",
            "markdown": f"This would create a PR with the generated rules at .watchflow/rules.yaml"
        },
        analysis_summary={
            "repository": repo_name,
            "total_prs_analyzed": 30,
            "recommendations_count": len(rules_to_use),
...
@router.post("/recommend-test-proceed", response_model=ProceedWithPRResponse)
async def proceed_with_pr_test(request: Request, payload: ProceedWithPRRequest, user: User | None = Depends(get_current_user_optional)) -> ProceedWithPRResponse:
    """
    Mock endpoint for testing PR creation without requiring GitHub API access.
    """
    repo_name = payload.repository_full_name
    
    return ProceedWithPRResponse(
        pull_request_url=f"https://github.com/{repo_name}/pull/42",
        pull_request_number=42,
        branch_name="watchflow/rules-automation",
        base_branch="main",
        file_path=".watchflow/rules.yaml",
