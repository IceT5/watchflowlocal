...
async def github_webhook_endpoint(
    ...
    normalized_event_name = event_name.split(".")[0]
    supported_event_types = EventProcessorFactory.get_supported_event_types()
    if normalized_event_name not in supported_event_types:
        logger.info(
            "webhook_ignored_unsupported_event",
            event_type=event_name,
            normalized=normalized_event_name,
        )
        return WebhookResponse(
            status="ignored",
            detail=f"Event type '{event_name}' is received but not supported.",
